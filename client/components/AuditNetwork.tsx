...
const AuditNetwork: React.FC<AuditNetworkProps> = ({ logs, onRefresh }) => {
  ...
  const getActionColor = (action: string) => {
    switch (action) {
      ...
      case 'STATUS_CHANGE': return 'bg-indigo-100 text-indigo-700 border-indigo-200';
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h2 className="text-2xl font-black text-slate-900 tracking-tight">Audit Network</h2>
            <p className="text-slate-500 text-xs font-bold uppercase tracking-widest mt-1">Real-time state immutable ledger</p>
          </div>
        </div>
        <button 
          onClick={onRefresh}
      ...
      <div className="bg-white rounded-[3rem] shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-10 border-b border-slate-50 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Global Activity Stream</span>
          </div>
          <span className="text-[10px] font-black text-slate-300 uppercase tracking-tighter">Displaying latest 100 entries</span>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50">
                ...
                <th className="px-10 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Actor</th>
                ...
                <th className="px-10 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">State Change (Old → New)</th>
              </tr>
            </thead>
                  ...
                  <tr key={log.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-10 py-6">
                      <div className="flex items-center space-x-3">
                        ...
                        <span className="text-xs font-black text-slate-700">{log.changedBy}</span>
                      </div>
                    </td>
                    <td className="px-10 py-6">
                    ...
                    <td className="px-10 py-6">
                      <span className={`px-3 py-1 rounded-full border text-[9px] font-black uppercase tracking-widest ${getActionColor(log.action)}`}>
                        {log.action}
                      </span>
                    </td>
                    <td className="px-10 py-6">
                      <div className="flex items-center space-x-3 max-w-xs">
                        <span className="text-[10px] font-bold text-slate-400 truncate bg-slate-50 px-2 py-1 rounded border border-slate-100 italic">
                          {oldState ? (oldState.status || oldState.name || oldState.amount || 'Initial') : 'Null'}
                        </span>
                        <ArrowRight size={10} className="text-slate-300 shrink-0" />
                        <span className="text-[10px] font-black text-slate-900 bg-indigo-50 px-2 py-1 rounded border border-indigo-100">
                          {newState ? (newState.status || newState.name || newState.amount || 'Updated') : 'Void'}
                        </span>
                      </div>
                      <p className="text-[8px] text-slate-400 font-bold uppercase mt-1 tracking-tighter">ID: {log.targetId}</p>
                    </td>
                  </tr>
                );
              })}
          ...
          {logs.length === 0 && (
            <div className="p-20 text-center">
              ...
              <p className="text-slate-400 font-black uppercase text-xs tracking-widest">Ledger is currently empty</p>
            </div>
          )}
        </div>
      </div>
