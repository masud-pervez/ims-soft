  ...
  try {
    ...
    for (const col of requiredCols) {
      if (!existingColNames.includes(col.name.toLowerCase())) {
        console.log(`MIGRATION: Adding missing column [${col.name}] to audit_logs...`);
        await initConn.query(`ALTER TABLE audit_logs ADD COLUMN \`${col.name}\` ${col.type} AFTER \`${col.after}\``);
      }
    }
